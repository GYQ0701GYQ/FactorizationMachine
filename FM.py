...
def SGD_FM(dataMatrix, classLabels, k, iter):
    ...
    :param k:           辅助向量的大小，即隐因子向量的维度
    :param iter:        迭代次数
    :return:
    '''
    # dataMatrix用的是mat, classLabels是列表
    m, n = shape(dataMatrix)  # 矩阵的行列数，即样本数和特征数，m个样本，n个特征
    alpha = 0.01
    # 初始化参数
    ...
    for it in range(iter):
        for x in range(m):  # 随机优化，每次只使用一个样本
            ...
            xx0 = dataMatrix[x]     # shape = (1,n)
            xx = np.array(xx0)  # xx是一个样本 shape = (1,n)
            xx1 = xx.T @ xx     # shape = (n,n)
            vv = v @ v.T   # shape = (n,n)
            e = xx1 * vv
            interaction = 0.5 * (e.sum() - e.trace())   # 各个特征做二阶交互，去掉矩阵的trace再乘1/2即为上三角
            p = w_0 + xx @ w + interaction  # p即为考虑了偏置、一阶特征、二阶特征交互的prediction
            # print(p.shape)
            loss = (1 - sigmoid(classLabels[x] * p[0, 0]))  # 计算损失
            w_0 = w_0 + alpha * loss * classLabels[x]

            for i in range(n):
                if dataMatrix[x, i] != 0:   # 第x个样本的第i个特征不为0
